...
import com.rivetlogic.quickquestions.model.permissions.PropsValues;
...
public class MBUtil {
	...
	public static void addPortletBreadcrumbEntries(MBCategory category,
		...
		for (MBCategory curCategory : ancestorCategories) {
			portletURL.setParameter("mbCategoryId",
					String.valueOf(curCategory.getCategoryId()));

			PortalUtil.addPortletBreadcrumbEntry(request,
					curCategory.getName(), portletURL.toString());
		}
		
		portletURL.setParameter(
				"categories", String.valueOf(category.getCategoryId()));
			portletURL.setParameter(
					"targetPage","view_main");
				portletURL.setParameter(
						"topLink","all");
				portletURL.setParameter(
						"isSearch","true");
		...
		PortalUtil.addPortletBreadcrumbEntry(request, category.getName(),
				portletURL.toString());
	}
	...
	public static void addPortletBreadcrumbEntries(MBMessage message,
		...
		if (message.getCategoryId() != MBCategoryConstants.DEFAULT_PARENT_CATEGORY_ID) {
			...
			addPortletBreadcrumbEntries(message.getCategory(), request,
					renderResponse);
		}
		...
		PortalUtil.addPortletBreadcrumbEntry(request, message.getSubject(),
				portletURL.toString());
	}

	public static String getAbsolutePath(PortletRequest portletRequest,
			long mbCategoryId) throws PortalException, SystemException {
	...
	public static String getEmailMessageAddedBody(PortletPreferences preferences) {
		...
		if (Validator.isNotNull(emailMessageAddedBody)) {
			...
			return ContentUtil
					.get(PropsValues.MESSAGE_BOARDS_EMAIL_MESSAGE_ADDED_BODY);
		}
	}
	...
	public static String getEmailMessageAddedSignature(
		...
		if (Validator.isNotNull(emailMessageAddedSignature)) {
			...
			return ContentUtil
					.get(PropsValues.MESSAGE_BOARDS_EMAIL_MESSAGE_ADDED_SIGNATURE);
		}
	}
	...
	public static String getEmailMessageAddedSubject(
		...
		if (Validator.isNotNull(emailMessageAddedSubject)) {
			...
			return ContentUtil
					.get(PropsValues.MESSAGE_BOARDS_EMAIL_MESSAGE_ADDED_SUBJECT);
		}
	}
	...
	public static String getEmailMessageUpdatedBody(
		...
		if (Validator.isNotNull(emailMessageUpdatedBody)) {
			...
			return ContentUtil
					.get(PropsValues.MESSAGE_BOARDS_EMAIL_MESSAGE_UPDATED_BODY);
		}
	}
	...
	public static String getEmailMessageUpdatedSignature(
		...
		if (Validator.isNotNull(emailMessageUpdatedSignature)) {
			...
			return ContentUtil
					.get(PropsValues.MESSAGE_BOARDS_EMAIL_MESSAGE_UPDATED_SIGNATURE);
		}
	}
	...
	public static String getEmailMessageUpdatedSubject(
		...
		if (Validator.isNotNull(emailMessageUpdatedSubject)) {
			...
			return ContentUtil
					.get(PropsValues.MESSAGE_BOARDS_EMAIL_MESSAGE_UPDATED_SUBJECT);
		}
	}
	...
	public static List<Object> getEntries(Hits hits) {
		...
		for (Document document : hits.getDocs()) {
			long categoryId = GetterUtil.getLong(document
			...
			try {
				...
				if (_log.isWarnEnabled()) {
					_log.warn("Message boards search index is stale and contains "
							+ "category " + categoryId);
				}
	...
	public static String getReplyToAddress(long categoryId, long messageId,
			String mx, String defaultMailingListAddress) {
	...
	public static String[] getThreadPriority(PortletPreferences preferences,
			String languageId, double value, ThemeDisplay themeDisplay)
			throws Exception {

		String[] priorities = LocalizationUtil.getPreferencesValues(
				preferences, "priorities", languageId);

		String[] priorityPair = _findThreadPriority(value, themeDisplay,
				priorities);
		...
		if (priorityPair == null) {
			...
			priorities = LocalizationUtil.getPreferencesValues(preferences,
					"priorities", defaultLanguageId);
	...
	public static Set<Long> getThreadSubscriptionClassPKs(long userId)
		...
		List<Subscription> subscriptions = SubscriptionLocalServiceUtil
				.getUserSubscriptions(userId, MBThread.class.getName());
	...
	public static String getUserRank(PortletPreferences preferences,
			String languageId, int posts) throws Exception {

		String rank = StringPool.BLANK;

		String[] ranks = LocalizationUtil.getPreferencesValues(preferences,
				"ranks", languageId);
	...
	public static String[] getUserRank(PortletPreferences preferences,
			String languageId, MBStatsUser statsUser) throws Exception {
		...
		for (int i = 0; i < ranks.length; i++) {
			...
			String[] curRankValueKvp = StringUtil.split(curRankValue,
					CharPool.COLON);
			...
			if (curRankValueKvp.length <= 1) {
				...
				try {
					if (_isEntityRank(companyId, statsUser, entityType,
							entityValue)) {
	...
	public static boolean isViewableMessage(ThemeDisplay themeDisplay,
			MBMessage message) throws Exception {

		return isViewableMessage(themeDisplay, message, message);
	}

	public static boolean isViewableMessage(ThemeDisplay themeDisplay,
			MBMessage message, MBMessage parentMessage) throws Exception {

		PermissionChecker permissionChecker = themeDisplay
				.getPermissionChecker();

		if (!MBMessagePermission.contains(permissionChecker, parentMessage,
				ActionKeys.VIEW)) {
		...
		if ((message.getMessageId() != parentMessage.getMessageId())
				&& !MBMessagePermission.contains(permissionChecker, message,
						ActionKeys.VIEW)) {
		...
		if (!message.isApproved()
				&& !Validator.equals(message.getUserId(),
						themeDisplay.getUserId())
				&& !permissionChecker.isGroupAdmin(themeDisplay
						.getScopeGroupId())) {
	...
	public static void propagatePermissions(long companyId, long groupId,
			long parentMessageId, ServiceContext serviceContext)
			throws PortalException, SystemException {

		MBMessage parentMessage = MBMessageLocalServiceUtil
				.getMBMessage(parentMessageId);

		Role defaultGroupRole = RoleLocalServiceUtil
				.getDefaultGroupRole(groupId);
		Role guestRole = RoleLocalServiceUtil.getRole(companyId,
				RoleConstants.GUEST);

		long[] roleIds = { defaultGroupRole.getRoleId(), guestRole.getRoleId() };

		List<String> actionIds = ResourceActionsUtil
				.getModelResourceActions(MBMessage.class.getName());

		Map<Long, Set<String>> roleIdsToActionIds = ResourcePermissionLocalServiceUtil
				.getAvailableResourcePermissionActionIds(companyId,
						MBMessage.class.getName(),
						ResourceConstants.SCOPE_INDIVIDUAL,
						String.valueOf(parentMessage.getMessageId()), roleIds,
						actionIds);

		Set<String> defaultGroupActionIds = roleIdsToActionIds
				.get(defaultGroupRole.getRoleId());

		if (defaultGroupActionIds == null) {
			serviceContext.setGroupPermissions(new String[] {});
		} else {
			serviceContext.setGroupPermissions(defaultGroupActionIds
					.toArray(new String[defaultGroupActionIds.size()]));
		}

		Set<String> guestActionIds = roleIdsToActionIds.get(guestRole
				.getRoleId());

		if (guestActionIds == null) {
			serviceContext.setGuestPermissions(new String[] {});
		} else {
			serviceContext.setGuestPermissions(guestActionIds
					.toArray(new String[guestActionIds.size()]));
		}
	}
	...
	public static String replaceMessageBodyPaths(ThemeDisplay themeDisplay,
		...
		return StringUtil.replace(messageBody, new String[] {
				"@theme_images_path@", "href=\"/", "src=\"/" },
				new String[] { themeDisplay.getPathThemeImages(),
	...
	public static void updateCategoryStatistics(long companyId,
		...
		Callable<Void> callable = new ShardCallable<Void>(companyId) {
			...
			protected Void doCall() throws Exception {
				...
				int threadCount = MBThreadLocalServiceUtil
						.getCategoryThreadsCount(category.getGroupId(),
	...
	public static void updateThreadMessageCount(long companyId,
		...
		Callable<Void> callable = new ShardCallable<Void>(companyId) {
			...
			protected Void doCall() throws Exception {
				...
				int messageCount = MBMessageLocalServiceUtil
						.getThreadMessagesCount(threadId,
								WorkflowConstants.STATUS_APPROVED);
	...
	private static String[] _findThreadPriority(double value,
			ThemeDisplay themeDisplay, String[] priorities) {
			...
			try {
				...
				if (value == priorityValue) {
					if (!priorityImage.startsWith(Http.HTTP)) {
						priorityImage = themeDisplay.getPathThemeImages()
								+ priorityImage;
					}
	...
	private static boolean _isEntityRank(long companyId, MBStatsUser statsUser,
			String entityType, String entityValue) throws Exception {
		...
		if (entityType.equals("organization-role")
			...
			if (UserGroupRoleLocalServiceUtil.hasUserGroupRole(userId, groupId,
					role.getRoleId(), true)) {
			...
			Organization organization = OrganizationLocalServiceUtil
					.getOrganization(companyId, entityValue);

			if (OrganizationLocalServiceUtil.hasUserOrganization(userId,
					organization.getOrganizationId(), false, false)) {

				return true;
			}
		} else if (entityType.equals("regular-role")) {
			if (RoleLocalServiceUtil.hasUserRole(userId, companyId,
					entityValue, true)) {
	...
	public static void setRenderParams(ActionRequest request, ActionResponse response){
